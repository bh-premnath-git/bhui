...
import nodeData from './data/node_display.json';
import schemaData from './data/mdata.json';
...
import NodeDropList from './BuildPipeLineComps/NodeDropList';
import { useParams } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { setSaving, setSaved, setSaveError, setUnsavedChanges } from '@/store/oldstore/features/autoSaveSlice';
...
const BuildPlayGround: React.FC = () => {
  ...
  const [conversionLogs, setConversionLogs] = useState<
    Array<{ timestamp: string; message: string; level: 'info' | 'error' | 'warning' }>
  >([]);
      ...
      try {
        ...
        if (response?.pipeline_json) {
          ...
          const nodesWithTitles = convertedJson.nodes.map((node: any) => {
            const matchingTransformation = response.pipeline_json.transformations.find((t: any) => {
          ...
          const initialFormStates: any = {};
          response.pipeline_json.transformations.forEach((transformation: any) => {
            const matchingNode = nodesWithTitles.find(
              (node: any) =>
                node.data.label === transformation.transformation && node.data.title === transformation.name
            );
              ...
              switch (transformation.transformation) {
                ...
                case 'Repartition':
                  initialFormStates[matchingNode.id] = {
                    ...
                    repartition_expression:
                      transformation.repartition_expression || [
                        {
                          expression: '',
                          sort_order: '',
                          order: 0,
                ...
                case 'Lookup':
                  initialFormStates[matchingNode.id] = {
                    ...
                    lookup_conditions:
                      transformation.lookup_conditions || [
                        {
                          source_column: '',
                          lookup_column: '',
                          operator: '=',
  ...
  const handleRunClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      const sourceNodes = allNodes.filter(
        (node) => node.data.label.toLowerCase().includes('source') || node.data.source
      );
      const targetNodes = allNodes.filter((node) => !edges.some((edge) => edge.source === node.id));
      ...
      const sources = sourceNodes.map((node) => ({
        ...
        connection: {
          ...
          connection_type:
            (node.data.source?.connection_type || 'local').charAt(0).toUpperCase() +
      ...
      const transformations = orderedNodes
        .map((node) => {
          ...
          if (node.data.label.toLowerCase().includes('source') || node.data.source) {
            ...
            return {
              ...
              source: {
                ...
                connection: {
                  name:
                    node.data.source?.custom_metadata?.source?.connection?.connection_name ||
                    node.data.source?.connection_type + '_connection' ||
                    'local_connection',
                  connection_type:
                    (node.data.source?.connection_type || 'local').charAt(0).toUpperCase() +
                    (node.data.source?.connection_type || 'local').slice(1),
          ...
          const dependentOn = incomingEdges
            .map((edge) => {
          ...
          if (node.data.label === 'Aggregator') {
            ...
            return {
              ...
              pivot:
                formState.pivot?.map((p: any) => ({
                  pivot_column: p.pivot_column,
                  pivot_values: p.pivot_values?.split(',').map((v: any) => v.trim()) || [],
                })) || [],
            };
          }
    ...
    [edges, formStates, reactFlowInstance, pipelineDtl, nodes]
  );
  ...
  const handleNodeForm = useCallback(
    (targetNodeId: string) => {
  ...
  const hasCycle = (graph: any, startNode: string, targetNode: string): boolean => {
    ...
    while (stack.length > 0) {
      ...
      if (graph[currentNode]) {
        graph[currentNode].forEach((neighbor: string) => {
  ...
  const handleSourceUpdate = useCallback(
    ({ nodeId, sourceData }: { nodeId: string; sourceData: any }) => {
      setNodes((prevNodes) =>
        prevNodes.map((node) => {
          if (node.id === nodeId) {
            return {
  ...
  const fetchSourceColumns = useCallback(async (nodes: any[]) => {
    try {
      ...
      const columnsPromises = sourceNodes.map(async (node) => {
        ...
        return (
          response[0]?.layout_fields?.map((field: any) => ({
  ...
  const handleSearchResultClick = useCallback(
    (nodeId: string) => {
      ...
      if (node) {
        reactFlowInstance.setCenter(node.position.x + 100, node.position.y + 100, { duration: 800 });
      }
    },
    ...
    <div>
      <div className="p-1 ml-8">
          ...
          <div className="mb-4 p-2 bg-blue-50 rounded-lg">
            ...
            <div className="flex flex-wrap gap-2">
              {debuggedNodesList.map(({ id, title }) => (
                ...
                >
                  <span>{title}</span>
                  <button onClick={() => handleDebugToggle(id, title)} className="hover:text-blue-900">
        ...
        <div className="flex justify-center gap-4 mb-4">
          <NodeDropList filteredNodes={filteredNodes} handleNodeClick={handleNodeClick} addNodeToHistory={addNodeToHistory} />
        </div>
        ...
        {/* -- DIALOG for FORM (was MUI Dialog) -- */}
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="w-[1000px] max-w-none">
            {/* 
              If desired, you can wrap in <DialogHeader> / <DialogTitle> 
              but here we'll just show the form 
            */}
            <CreateFormFormik
        ...
        {/* DIALOG for "Leave Page?" prompt */}
        <Dialog
          open={showLeavePrompt}
          // We want to mimic MUI's onClose => handleLeavePage:
          // If user clicks outside or presses ESC, it calls handleLeavePage.
          // So we do onOpenChange => if open is false => handleLeavePage
          onOpenChange={(open) => {
            if (!open) {
              handleLeavePage();
            }
        ...
        >
          {/* 
            We replicate MUI's style with Tailwind:
            borderRadius: 12px => rounded-[12px]
            padding: 8px => p-2 or p-3
            maxWidth: 450px => max-w-[450px]
            boxShadow => shadow-[0_8px_32px_rgba(0,0,0,0.08)]
          */}
          <DialogContent className="rounded-[12px] p-6 max-w-[450px] shadow-[0_8px_32px_rgba(0,0,0,0.08)]">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Unsaved Changes</h2>
              <p className="text-gray-600 mb-6">
                You have unsaved changes in your pipeline. Are you sure you want to leave? All
