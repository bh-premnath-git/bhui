...
export const useLayoutPersistence = ({ storageKey, dashboard }: LayoutConfig) => {
  const [layouts, setLayouts] = useState<Layouts>(() => {
    ...
    if (!dashboard?.dashboard_layout) return {};
    ...
    return {
      lg: sortedLayout.map((layout, index) => ({
        ...
        x: layout.widget_coordinates.x || (index % 2) * 6,
        y: layout.widget_coordinates.y || Math.floor(index / 2) * 3,
        w: layout.widget_size.w || 6,
        h: layout.widget_size.h || 3,
        minW: 3,
        minH: 2,
        maxW: 12,
        maxH: 6,
