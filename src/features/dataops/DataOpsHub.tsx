...
export function DataOpsHub() {
  ...
  const dashboardLayoutMap = useMemo(() => {
    ...
    return new Map<string, { layout_id: string; x: number; y: number; w: number; h: number; order_index: string }>(
      dl.map((it: any) => [
        String(it.widget_id),
        {
      ...
      if (!widgets[widgetId]) {
        ...
        dispatch(
          addWidget({
            ...rest,
            id: widgetId,
            layout: layoutMap.get(widgetId), // use layoutMap from store
          })
        );
      }
    });
