            ...
            conversation.map((entry) => (
              <div key={entry.id} className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 bg-white p-4 rounded-lg shadow-md">{entry.question}</div>
                </div>
  
                {entry.response === null ? (
                  <div className="flex items-center gap-3 pl-10 text-gray-400 animate-pulse">
                    <Loader2 className="h-6 w-6" />
                    <span>Loading...</span>
                  </div>
                ) : (
                  <div className="flex items-start gap-4">
                    <img src="/assets/buildPipeline/bighammer.png" alt="bighammer" className="w-10 h-10" />
