...
export const useXplore = (options: UseXploreOptions = { shouldFetch: true }) => {
  ...
  const streamConversation = useCallback(
    (conversationId: number, userRequest: string, threadId: string, onChunk: StreamCallback, onComplete?: () => void, onError?: (error: any) => void): () => void => {
      // Create controller for aborting if needed
      const controller = new AbortController();
      const { signal } = controller;
      
      // Build the URL 
      const baseUrl = process.env.NEXT_PUBLIC_API_HOST || window.location.origin;
      const url = `${baseUrl}/api/agent/conversation/conversation/query/stream?connection_config_id=${conversationId}`;
      ...
      // Return a cleanup function that aborts the fetch if needed
