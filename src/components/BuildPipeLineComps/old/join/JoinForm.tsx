...
function convertLabel(input: string) {
        ...
        <>
            <div className="flex space-between">
                <div className=" ml-4 w-96">
                    <Label>{convertLabel(form.fields[0].name)}</Label>
                </div>
                <div className=" ml-4 w-96">
                    <Label>{convertLabel(form.fields[1].name)}</Label>
                </div>
            </div>
            <Formik
                initialValues={{
                                            ...
                                            <div key={index} className="flex space-between">
                                                {form.fields.map((field: any, idx: any) => (
                                                    <div key={idx} className="ml-4">
                                                        {field.type === "select" ? (
                                                            <div className="w-96">
                                                                <CustomField options={joinList} name={`conditions.${index}.${field.name}`}
                                                                    controlName="select"
                                                                    //  label={field.label}
                                                                    labelKey="dtl_desc" valueKey="codes_hdr_id" />
                                                            </div>
                                                        ) : (
                                                            <div className="w-96">
                                                                <CustomField
