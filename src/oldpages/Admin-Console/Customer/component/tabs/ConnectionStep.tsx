...
function ConnectionStep(props: any) {
	...
	const fetchConnectionField = async (id, index) => {
		try {
	...
	const handleClick = async (values, index) => {
	...
	const handleChange = (index, setFieldValue, event) => {
		setIsTestConnection(false)
		const platformCode = event.target.value;
		const id = tragetPlatformList.find(platform => platform.id === platformCode)?.dtl_id_filter;
		setFieldValue(`connections[${index}].target_delivery_platform_cd`, platformCode);
		fetchConnectionField(id, index);
	};

	const renderDynamicFields = (fields, index) => {
		return <Stack direction={'row'}>
			{fields.map((field: any) => (
				<div key={field.id} style={{ width: '100%', margin: '10px 10px' }}>
					...
					<Field as={TextField} name={`connections[${index}].connection_details.${field.dtl_desc.toLowerCase().replace(' ', '_')}`} placeholder={convertToReadableFormat(field.dtl_desc)} fullWidth />
										...
										<AccordionDetails>
											<Stack direction={'row'}>
												<Stack width={'100%'} className='px-1'>
													...
													<Field as={TextField} name={`connections[${index}].connection_name`} placeholder="Connection Name" fullWidth />
													<div style={{ color: 'red', textAlign: 'start' }}>
												...
												<Stack width={'100%'} className='px-1'>
													...
													<Field
														...
														render={({ field }) => (
															...
															>
																{tragetPlatformList?.map((targetPlatform) => (
