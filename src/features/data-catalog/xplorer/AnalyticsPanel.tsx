...
import { useDashboard } from "@/context/XploreDashboardContext";
...
export default function AnalyticsPanel() {
  ...
  const handleSave = (type: 'existing' | 'new') => {
    ...
    toast.success(`Dashboard saved ${type === 'new' ? 'as new' : 'to existing'} successfully.`);
  ...
  const handleFilterClick = (brand: string) => {
    setActiveFilters((prevFilters: string[]) => {
      ...
      if (isSelected) {
        ...
        toast.success(`Removed ${brand} from filters`);
        return newFilters;
      } else {
