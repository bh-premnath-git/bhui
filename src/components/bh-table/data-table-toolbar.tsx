...
export function DataTableToolbar<TData>({
  ...
  // Determine if any filters (global or column) are active
  const isFiltered =
  ...
  // If using a status card view, render that instead of the normal toolbar.
  if (useStatusCard) {
    return (
      <DataTableStatusFilter
        data={{
          success: table.getPreFilteredRowModel().rows.filter(
            (row) => row.getValue("status") === "success"
          ).length,
          failed: table.getPreFilteredRowModel().rows.filter(
            (row) => row.getValue("status") === "failed"
          ).length,
          inProgress: table.getPreFilteredRowModel().rows.filter(
            (row) => row.getValue("status") === "in progress"
          ).length,
        }}
        selectedStatuses={
        ...
        onStatusSelect={(status: string) => {
          ...
          if (currentFilters.includes(status)) {
            table
              .getColumn("status")
              ?.setFilterValue(currentFilters.filter((s: string) => s !== status));
          } else {
